...
void Thread::notify_friends()
{
	this->threadPCB->friends_queue->notify_friends();
}
int Thread::friends_started()
{
	return this->threadPCB->friends_queue->count_started_friend();
}
void Thread::makeFriends(Thread* t1,Thread* t2)
{
	t1->threadPCB->friends_queue->put(t2->threadPCB);
	t2->threadPCB->friends_queue->put(t1->threadPCB);
}
ID Thread::getId()
{
...
void Thread::start()
{
	...
	lock();
#endif
	if(this->friends_started()<=2)
	{
		syncPrintf("proso start %d\n",(int)getId());
		threadPCB->myPcbState = ready;
		threadPCB->createStack();
		Scheduler::put(threadPCB);
	}
	else {
		syncPrintf("NIJE proso start %d\n",(int)getId());
		threadPCB->myPcbState = tried_to_start;
