...
int PcbSem::wait(unsigned TimeSlice)
{
    ...
    if(val<0)
    {
    	...
    	else
    	{
    		if(TimeSlice==0)
    		{
    			syncPrintf("Blokiram %d,%d\n",(int)toBlock->myThread->getId(),toBlock->myThread->myPrior);
    			if(Timer::testingPhase)
...
int PcbSem::signal(int signalValue) {
    ...
    {
    	 myWaitQueue->sort();
    	 //myWaitQueue->printQueueElements();
    	 for(int i=0;i<maxThreadstoUnblock;i++)
    	 {
    		 if(myWaitQueue->isEmpty()==0)
    		 {
    			 ...
    			 unblock(toUnblock);
    			 syncPrintf("[[unblock]]-> %d,%d\n",(int)toUnblock->myThread->getId(),toUnblock->myThread->myPrior);
