...
#include "Thread.h"
#include "SCHEDULE.H"
void queue::notify_friends()
{
	qnode* temp = firstNode;
	int return_val=0;
	while(temp!=0)
	{
		if(temp->element->myPcbState==tried_to_start && (temp->element->friends_queue->count_started_friend()<=2))
		{
			syncPrintf("Automatic start %d\n",(int)temp->element->myThread->getId());
			temp->element->myPcbState = ready;
			temp->element->createStack();
			Scheduler::put(temp->element);
		}
		temp=temp->next;
	}
}
int queue::count_started_friend()
{
	...
	while(temp!=0)
	{
		if(temp->element->myPcbState==ready || temp->element->myPcbState==blocked) return_val++;
		temp=temp->next;
	}
	//syncPrintf("%d, Friend started->%d\n",(int)Timer::running->myThread->getId(),return_val);
#ifndef BCC_BLOCK_IGNORE
	unlock();
#endif
	return return_val;
