...
int queue::hasBlocked(queue* friends_queue)
{
	qnode* temp = firstNode;
    qnode* temp2 = friends_queue->firstNode;

    while(temp!=0)
    {
    	if( friends_queue->getThreadByIdSearchQueue(temp->element->myThread->getId()) != 0)
    	{
    			return (int)temp->element->myThread->getId();
    	}
    	temp = temp ->next;
    }
    return 0;
}
//q = threads_in_critical
//=1 -> treba da se blokiram
//=0 ne treba da se blokiram
int queue::check(queue* q)
{
	if(q->queueLen==0) return 0;
	int return_val =1;
	qnode* temp = firstNode;

	qnode* temp2 = q->firstNode;

	while(temp2!=0)
	{
		if(this->getThreadByIdSearchQueue(temp2->element->myThread->getId()) == 0)
		{
			return 1;
		}
		temp2=temp2->next;
...
PCB* queue::get_by_id(ID id)
{
	...
	while (cur) {
		...
		if (cur->element->myThread->getId() != id) {
			...
			el = cur;
			el_for_return = el->element;
			//syncPrintf("%d[][][]\n",(int)el_for_return->myThread->getId());
			old = cur;
			cur = cur->next;
			if (!prev)
				firstNode = cur;
