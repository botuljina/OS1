...
void queue::printQueueElements()
{
	...
	while(temp!=0)
	{
		if(temp!=0)syncPrintf("%d,%d\n",(int)temp->element->myThread->getId(),temp->element->myThread->myPrior);
		temp=temp->next;
	}
	syncPrintf("\n************* \n");
#ifndef BCC_BLOCK_IGNORE
	...
	int swapped;
		...
		do
		{
			...
			while(lPtr->next != rPrt)
			{
				...
				if (lPtr->element->myThread->myPrior < lPtr->next->element->myThread->myPrior)
				{
