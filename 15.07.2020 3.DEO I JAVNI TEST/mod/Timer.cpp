...
semQueue* Timer::mySemQueue = new semQueue();
//---------------------------------------------------------
...
void interrupt Timer::timerRoutine(...)
{
	...
	if (newThread == 0) newThread = idleThread->threadPCB; //ako je prazan Scheduler, uzimaj idle nit

	//if (newThread->myPcbState != ready) continue;
...
void Timer::restore_system() {
	...
	delete idleThread;
	delete mySemQueue;
	if(testingPhase) syncPrintf("EKSPLICITNI DELETE JE SVUGDE POZVAN!!!");
#ifndef BCC_BLOCK_IGNORE
	unlock();
#endif
}
...
void Timer::wrapper() {
	...
	//if(testingPhase)
	//	syncPrintf("zavrsio se run za nit sa id-jem %d i vadim iz njenog queue-a\n",running->myThread->threadID);
	//prelazak na drugu nit, zato sto na steku nema povratne adrese za povratak iz wrapper-a
	dispatch();
}
